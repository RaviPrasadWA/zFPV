...
static int get_recording_index_track_count() {
  const std::string recording_track_filename =
      std::string(openhd::RECORDINGS_BASE_PATH) + "track_count.txt";
  int track_count = 1;
...
static std::string create_unused_recording_filename(const std::string& suffix) {
  if (!OHDFilesystemUtil::exists(std::string(openhd::RECORDINGS_BASE_PATH))) {
    OHDFilesystemUtil::create_directories(std::string(openhd::RECORDINGS_BASE_PATH));
  }
  ...
  std::stringstream ss;
  ss << std::string(openhd::RECORDINGS_BASE_PATH) << "recording_" << track_index << suffix;
  return ss.str();
