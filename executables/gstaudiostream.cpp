...
#include "video_air_generic_settings.h"
...
std::string GstAudioStream::create_pipeline() {
  ...
  auto opt_manual_audio_source = OHDFilesystemUtil::opt_read_file(
      std::string(openhd::SETTINGS_BASE_PATH) + "audio_source.txt", false);
  // audiotestsrc always works, but obviously is not a mic ;)
  if (OHDFilesystemUtil::exists(std::string(openhd::SETTINGS_BASE_PATH) +
                                "test_audio.txt") ||
